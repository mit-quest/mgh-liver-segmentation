...
def count_fat_macro_micro(image, mask):
    ...
    for row in range(len(image_255)):
        for col in range(len(image_255[row])):
            if image_255[row][col] <= 60 and not (170 <= row <= 520 and 70 <= col <= 410): # 45 experimentally determined, changed to 60 for frozen
...
def frozen_liver_segmentation(images_directory, output_directory, liver_name, image_name, ij):
    ...
    _, sharpened_binary = cv2.threshold(sharpened_gray, 205, 255, cv2.THRESH_BINARY)

    # Remove noise
    opened_image_bool = remove_small_holes(sharpened_binary, area_threshold=10) # Change black area < 5 pixels to white
    ...
    # Apply erosion before watershed
    ...
    cv2.imwrite(save_path, erode_image)
    ...
    new_mask = get_fat_score_for_watershed_image(islands, np_graph, min_size=2, max_size=2000) # With erosion, adjust size thresholds
...
def main(**args: Dict[str, Any]) -> None:
    ...
    for liver_name in liver_folders:
        ...
        for image_name in liver_images:
                frozen_liver_segmentation(images_directory, output_directory, liver_name, image_name, ij)
