...
def remove_background(opened_image_bool):
    ...
    for island in islands:
        if len(island) > 0:
            ...
            if len(island) < 1000 and ratio > 0.17: # 1000 good for formalin HF-3 liver, 0.17 determined experimentally
...
def calculate_large_white_area(image_path):
    ...
    """
    image, sharpened_image = sharpen_image(image_path, SHARPEN_FILTER)
    ...
    opened_image_bool = remove_small_holes(sharpened_binary, area_threshold=2) # Change black area < 5 pixels to white
    ...
    for island in islands:
        ...
        if island_len > 0:
            ...
            if not (island_len < 1000 and ratio > 0.17) and island_len > 70:
...
def liver_segmentation(images_directory, output_directory, liver_name, image_name, ij):
    ...
    original_image, sharpened_image = sharpen_image(image_path, SHARPEN_FILTER)
    sharpened_gray = cv2.cvtColor(sharpened_image, cv2.COLOR_BGR2GRAY)
    _, sharpened_binary = cv2.threshold(sharpened_gray, 195, 255, cv2.THRESH_BINARY)
